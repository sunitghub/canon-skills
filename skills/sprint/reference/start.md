...
Start checklist — rules that need no reference read (each owned by the step named):
- Pass the real ticket id, never a paraphrased title (step 1).
- Never write ticket status directly; `sprint`/`tkt` own it (step 1, `SKILL.md`).
- A template-only Description is not "unspecified" — acceptance.md/plan.md may hold the contract (step 4).

1. **Ticket and context.** Run `sprint start "<title>"` or `sprint start <ticket-id>`. If the request already names an existing ticket, pass that ID verbatim — only pass a title when no ticket exists yet. Per `sprint start --help`: a title creates a new ticket; an ID starts the sprint on the existing one, no duplicate created. It matches its argument against existing tickets first, so a paraphrased title instead of the real ID causes a false miss and a duplicate ticket. `sprint start` creates/starts the ticket, marks it active, ensures `DECISIONS.md`/`HANDOFF.md` exist, and seeds `acceptance.md`/`plan.md` skeletons (see Planning Files) if absent. Once started, **read the ticket's own docs before drafting a plan or asking the user for direction**: `.tickets/<id>/ticket.md` (the Description), `acceptance.md`, and `plan.md` — plus `research.md` if present. Read `tools/ticket.md` (the `tkt`/`sprint` CLI help — a **different file** from the ticket's own `ticket.md`, which holds the Description and frontmatter) only if you need a command you don't know: a plain relative path inside canon itself; inside a consumer project (`tools/` not symlinked in), find it via `command -v sprint`'s containing directory instead (`where sprint` on Windows if `command -v` returns nothing) — `ticket.md` sits beside `sprint` there. An empty or template-only Description does not mean the work is unspecified — the build contract may live entirely in `acceptance.md`/`plan.md` (see Planning Files).

2. **Skill check.** Run `./tools/skills.sh list` (or `skills.sh list` via `command -v sprint`'s directory in a consumer project where `tools/` isn't symlinked in) and ask: does an existing skill already cover this work? If yes, use it — don't reinvent. If no skill covers it and the work is reusable across projects, note it as a candidate for a new skill in `plan.md`. Building a new skill follows the same sprint flow as any other work.
...
4. **Planning files.** Both files were already seeded by step 1 (skeleton `## Criteria`/`## Test Plan`/`## QA` in `acceptance.md`, skeleton `## Sign-off`/`## Approach`/`## Files`/`## Decisions` in `plan.md`) — fill them in now, before the brief. The approval gate in step 11 blocks code, not planning file content.
   ...
   - **A criterion may not depend on an artifact the close sequence has not produced yet.** Criteria are graded by the evaluator at `complete.md` **step 3**, so anything a criterion cites must already exist. `summary.md` is written at **step 8**, so a criterion requiring something "recorded in `summary.md`" demands what cannot be there and fails correctly. `eval-report.md` is the grading step's *own* output, so citing it is circular. `review-notes.md` does exist by then (step 2) but is advisory and skipped on the `bugfix` and `demo` paths, so leaning on it makes a criterion tier-dependent. Cite instead: `acceptance.md` itself, `plan.md`, `research.md`, the code and its output, or an evidence file this sprint writes during implementation.
   - **A criterion referencing a gitignored file (e.g. `DECISIONS.md`) must say so explicitly**
     and instruct the evaluator to read it directly — `eval.md`'s changed-files-list method
     (`git diff --name-only`) can never see a file that's gitignored, so an unqualified
     reference to one is an ungradeable criterion, not a real gap (`t-bcbc`).
   ...
   - **Untrusted-input criterion (required when the sprint adds a tool, endpoint, or parser that
     ...
     this kind, or any sprint of this kind whose blast radius is real, plan the optional advisory break-it gate too
     (`reference/break-it.md` says when).
   ...
   - Read `standards/ticket-layout.md` (field contract, doc lifecycle, board rendering rules) only when a field, doc type, or rendering rule is unclear.
...
5. **Context.** Read in order:
   ...
   - `HANDOFF.md` — read current state and discoveries; if absent, create it from the template in `tools/handoff.md` (read that only then, or when you will write to `HANDOFF.md` — resolved like step 1's `tools/ticket.md`: plain relative path in canon itself, or via `command -v sprint`'s directory in a consumer project, or `where sprint` on Windows if `command -v` returns nothing)
   - `LEARNINGS.md` at repo root, if present — pending/promoted findings from past sprints (see `skills/learnings-sweep/SKILL.md`). Absent is a valid result: no sprint has produced a keeper yet.
   - Active sprint files
