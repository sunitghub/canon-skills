...
6. **Grade criteria.** For each item under `## Criteria` in `acceptance.md`:
   ...
   - For a **scenario-backed criterion** — one that carries a Given/When/Then block (an inline ` ```gherkin ` block per `t-6e32`, or a ` ```gherkin-file ` reference to a ticket-local `.feature` per `t-f89a`) — grade by **running the runner command named in that criterion's `## Test Plan` item and reading its exit code** (`pass` iff the command exits `0`, `fail` on non-zero), not by static reading of the implementation. When the ` ```gherkin-file ` reference carries a `runner: <cmd>` line (`t-6f8e`), that line is the authoritative source of the command: form and run `<runner> <feature-path>` from it (it is the same command the board displays and the `## Test Plan` item should name). Cite the exact command you ran and its result. This is the executable-spec case: the criterion is an input→output claim, so "run it, report the boolean" replaces "read and judge" — the same shift the visual rule makes toward rendered output. The **Weak Evidence** discipline still applies: a runner that does not actually exercise the changed code (e.g. one that trivially exits `0`, or never imports/calls what shipped) is weak evidence, not `pass`. And if the scenario was evidently authored or edited *alongside* the implementation rather than **locked at the sprint-start approval gate** before code, say so in Findings — a spec written to match the code it checks is a regression lock, not a correctness proof.
