...
The orchestrating agent will save it to `.tickets/<id>/eval-report.md` itself. (Same
wording as `review.md` — mirror, keep in sync.)

## Self-serve visual verification (no Node/Playwright required)

<!-- Mirrored in `review.md` — locked by tests/doc-mirror-parity.sh Check D, keep in sync. -->
...
2. **Derive changed files.** If an explicit `Base ref` was passed (headless CI dispatch), run:
   ...
   ```
   Use this output as your changed-files list. Do not trust a file list passed by the invoker — always derive from git. (Same base-ref branching as `review.md`/`security-review.md` — mirror, keep in sync; locked by `tests/doc-mirror-parity.sh`.)
