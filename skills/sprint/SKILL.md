...
`skills/` tree. Known gap: if `.claude/skills` already existed as a real directory before
`skills.sh add` ran, a hidden sub-skill like `wrapup` may not be reachable via `.claude/skills` —
...
board surfaces (New-Ticket **Demo/Docs/UX** option, Plan-tab toggle) are shipped — the headless `ci + demo` guard/warning (Phase C) is the only piece still outstanding.
...
.tickets/<id>/
  ...
  learnings.md     ← optional UNPROMOTED lessons candidate; written by `tkt learn` from this sprint's deviations + evaluator and reviewer findings, for a non-builder to promote; never close-gated
