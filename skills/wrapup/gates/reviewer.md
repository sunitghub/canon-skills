...
6. **Security (shallow):** no obviously unsafe patterns; destructive actions enforce server-side auth and consistent guards. This is a *shallow* pass — deep/exploit-level analysis is deferred to `security-review` (step 3 of the wrapup pipeline, not `complete.md`'s own step 3, the Evaluator); don't duplicate it here.
