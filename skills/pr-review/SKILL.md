...
- **Existing entry (PR number already in table):** update the row in place — replace Findings and Verdict with the current analysis. Do not add a new row.
- **New entry:** insert a new row at the top of the table (below the header), above existing rows. Set Verdict to `Merge` (Fix=Y) or `No` (Fix=N).
