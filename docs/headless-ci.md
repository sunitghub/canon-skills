...
- **Not the full wrapup pipeline.** `code-simplifier`/`code-reviewer`/`repo-check`/`doc-audit` are quality/consistency gates, not safety-critical, and aren't run headlessly — for **any** tier, not just `bugfix`. `sprint-headless` is a 3-gate dispatcher (reviewer/evaluator/security-review), not a full reproduction of the interactive wrapup pipeline.
