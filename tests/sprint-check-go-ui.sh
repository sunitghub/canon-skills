...
(
  ...
  SPRINT_CHECK_TEST_ROOT="$WORK" \
    npx playwright test tests/sprint-check-app.spec.js --grep "clicking an in-progress card|hovering the ready indicator|plan approach without sign-off|unchecked QA box blocks ready|editing docs works for quoted numeric ticket ids|archive button|modal next"
)
