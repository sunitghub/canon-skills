...
4. **Mechanical close gate.** The CLI refuses to close while Acceptance or Test Plan items are unchecked, `summary.md` is missing, the Wrapup Gates record is absent, the evaluator run-id field is missing, the eval verdict isn't `pass` (a `fail` or `partial` verdict blocks close the same way), or `plan.md`'s Approach/Sign-off sections are empty or unapproved. Gates don't make agents smarter — they make certain failures impossible.
