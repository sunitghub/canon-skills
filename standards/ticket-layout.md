...
| `demo` | bool | `tkt` | Optional. `true` puts `sprint complete` on the **demo close-path**: keep only `security-review` + the binding evaluator (forced Haiku unless an explicit `Gate model:` is set), skip the advisory reviewer + the rest of wrapup, with loud `skipped \| demo mode` Wrapup Gates rows and a `summary.md` demo line. Absent by default. It **never drops below the binding evaluator** and changes no CLI gate — the reduction is a user-flag exception to the "only structural risk reduces gates" invariant (see `AGENTS.md`'s north-star exception + `DECISIONS.md` 2026-07-30). Headless/CI ignores `demo`. Phase B: hand-set in frontmatter; `tkt demo`/board surfaces are Phase A. |
